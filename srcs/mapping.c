...
int fa_map_file_into_memory(const char *filename)
{
    ...
    else
    {
        ...
        if (res < 0 || res > 2147483647)
            ...
            stockhlm_header.original_filesize = res;
        printf("res: %ld\n", res);
    ...
    */
    printf("111duiffdildsfjqsfwodg...\n");

    g_mapped_data =
        mmap(NULL, stockhlm_header.original_filesize, PROT_READ | PROT_WRITE, MAP_PRIVATE, fd, 0);
    if (g_mapped_data == MAP_FAILED)
    {
        close(fd);
        return 1;
    }
    printf("222qsduiffdildsfjqsfwodg...\n");

    close(fd); /* No need to keep the fd since the file is mapped */

    // Abort if the current target already contains our signature
    if (search_binary((char *)g_mapped_data, stockhlm_header.original_filesize, FA_SIGNATURE, strlen(FA_SIGNATURE)))
        return 1;
    return 0;
}
...
int fa_write_processed_data_to_file(char *target_path)
{
    ...
    int outfilefd = open(target_path, O_CREAT | O_RDWR | O_TRUNC, 0755);
    // Check if open() has failed
    if (outfilefd == 1)
        return 1;

    // Write the processed data to the outfile
    ssize_t bytes_written = write(outfilefd, g_mapped_data, stockhlm_header.original_filesize);
    if (bytes_written < 0)
        return 1;
    if (munmap(g_mapped_data, stockhlm_header.original_filesize) < 0)
        return 1;
    close(outfilefd);
    return 0;
}
